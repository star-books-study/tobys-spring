...
#### 테스트 클래스의 컨텍스트 공유
- 스프링은 테스트 클래스 안에서 뿐만 아니라 테스트 클래스가 사이에서도 애플리케이션 컨텍스트를 공유하게 해준다.
- 두 개의 테스트 클래스가 같은 설정파일을 사용하는 경우
	- 테스트 수행 중에 단 한 개의 애플리케이션 컨텍스트만 만들어진다.
	- 두 테스트 클래스가 한 애플리케이션 컨텍스트 공유하게 되는 것

#### @Autowired
- DI에 사용되는 특별한 애너테이션
- @Autowird가 붙은 인스턴스 변수가 있으면 테스트 컨텍스트 프레임워크는 변수 타입과 일치하는 컨텍스트 내의 빈을 찾는다.
- 타입이 일치하는 빈이 있으면 인스턴스 변수에 주입해준다.
- 애플리케이션 컨텍스트는 초기화할 때 자기 자신도 빈으로 등록했다. 왜일까?
  - ApplicationContext 타입에 @Autowired를 붙였을 때 애플리케이션 컨텍스트가 DI 된다.
```java
...
- 단, @Autowired는 같은 타입의 빈이 두 개 이상 있는 경우에는 타입만으로는 어떤 빈을 가져올지 결정할 수 없다.
	- 따라서 테스트에서 가능한 한 인터페이스를 사용해서 애플리케이션 코드와 느슨하게 연결해두는 편이 좋다.

### 2.4.2 DI와 테스트
- 절대로 DataSource의 구현 클래스를 바꾸지 않는 상황이라고 가정한다면, 그냥 SimpleDriverDataSource를 생성하고 사용하면 안 될까?
	=> 그래도 인터페이스를 두고 DI를 적용해야 한다.
- 이유
	1. 소프트웨어 개발에서 절대로 바뀌지 않는 것은 없기 때문이다.
	2. 클래스의 구현 방식은 바뀌지 않는다고 하더라도 인터페이스를 두고 DI를 적용하게 해두면 다른 차원의 서비스 기능을 도입할 수 있기 때문이다. (ex. DB 커넥션의 개수를 카운팅하는 부가기능)
	3. 테스트

	   자동으로 실행 가능하며 빠르게 동작하도록 테스트 코드를 만들려면 가능한 한 작은 단위의 대상에 국한해서 테스트해야 한다. DI는 테스트가 작은 단위의 대상에 대해 독립적으로 만들어지고 실행되게 한다.
#### 테스트 코드에 의한 DI
...
=> DI를 이용해서 테스트 중에 DAO가 사용할 DataSource 오브젝트를 바꿔준다.

- 테스트용 DB에 연결해주는 DataSource를 테스트 내에서 직접 만들면 된다.
- 이렇게 하면 XML 설정파일을 수정하지 않고도 테스트 코드를 통해 오브젝트 관계를 재구성할 수 있다.
...
- @DirtiesContext
  - 이 애노테이션은 스프링의 테스트 컨텍스트 프레임워크에게 해당 클래스의 테스트에서 애플리케이션 컨텍스트의 상태를 변경한다는 것을 알려준다.
	...
	- 테스트에서 빈의 의존관계를 강제로 DI 하는 방법을 사용했을 때 문제는 일단 피할 수 있다.
> `@DirtiesContext`는 메소드 레벨로도 적용할 수 있다.
...
번거롭게 수동 DI 하는 코드나 `@DirtiesContext`도 필요 없다.
...
- 그에 비해 테스트 코드를 만들면서 하는 학습은 흥미롭고 재미있다.
...
- **버그 테스트(bug test)** : 코드에 오류가 있을 때 그 오류를 가장 잘 드러내줄 수 있는 테스트를 말한다.
